...
Quando o provedor ativo for LDAP, o sistema DEVE obter conexão e escopo de busca a partir de variáveis de ambiente (documentadas), incluindo URI do servidor, base de usuários, filtro de pesquisa com substituição do nome de usuário informado, e opções de TLS/startTLS conforme suportado pela implementação.
...
O sistema DEVE validar a senha do usuário usando o diretório LDAP de forma segura (por exemplo bind com credenciais do usuário após resolver o DN), e NÃO DEVE aceitar credenciais incorretas como sucesso.
